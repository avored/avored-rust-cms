...
const AdminUserTable: React.FC = () => {
  const getFormattedDate = (date: string): string => {
    ...
    }-${dateObject.getDate()}`; // Month is 0-indexed, so add 1
  };

  const adminUserTableResponse = useAdminUserTable();
  const adminUsers: AdminUser[] = _.get(
    adminUserTableResponse,
    "data.data.data",
    []
  );
          ...
          <table className="w-full bg-white shadow-md rounded">
            <thead>
              <tr className="bg-gray-700 text-white">
                ...
                <th className="py-3 px-4 rounded-r font-semibold text-left">
                  {t("common.action")}
                </th>
              </tr>
            </thead>
