...
interface Page {
  ...
  created_by: string;
  updated_by: string;
  components_content: ComponentContent[];
}

interface Component {
  map(
    arg0: (component: Component) => import("react/jsx-runtime").JSX.Element
  ): import("react").ReactNode | Iterable<import("react").ReactNode>;
  component_fields_content: any;
  id: number;
  name: string;
  identifier: string;
  fields: any[];
...
interface ComponentFieldContent {
  id: number;
  name: string;
  identifier: string;
  field_type: string;
...
function PageEdit() {
  ...
  useMemo(() => {
    const pageData: Page = _.get(data, "data.page_model");

    const comms: Component[] = _.get(pageData, "components_content", []).map(
      (componentContent) => ({
        ...componentContent,
        fields: [], // Add missing property
        created_at: "", // Add missing property
        updated_at: "", // Add missing property
        created_by: "", // Add missing property
        updated_by: "", // Add missing property
        component_fields_content: [], // Add missing property
        map: () => null, // Return null instead of void
      })
    );
    comms.forEach((com) => {
  ...
  const componentFieldContentOnChange = (
  ...
  const renderComponentFieldType = (componentField: ComponentField) => {
  ...
  const renderComponentField = (componentField: ComponentField) => {
    return (
  ...
  const componentSelected = (
    ...
    e.preventDefault();
    const selectedComponent = componentId.find(
  ...
  const pageNameOnChange = (value: string) => {
    if (page) {
      setPage({
        ...page,
        name: value,
  ...
  const pageIdentifierOnChange = (value: string) => {
    if (page) {
      setPage({
        ...page,
        identifier: value,
  ...
  const pageAddComponentSelected = (component: Component) => {
    var componentContent = {} as ComponentContent;
    ...
    component.fields.forEach((field) => {
      var componentFieldContent = {} as ComponentFieldContent;
  ...
  const renderComponent = (pageComponent: Component) => {
    return (
      <div
      ...
      >
        ...
        <div>
          Component Fields
          {pageComponent.component_fields_content.map(
