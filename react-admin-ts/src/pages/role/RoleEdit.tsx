...
function RoleEdit() {
  ...
  const { mutate } = useUpdateRole(params.role_id || ""); // Provide a default value of an empty string if params.role_id is undefined
  const { data } = useGetRole(params.role_id || ""); // Provide a default value of an empty string if params.role_id is undefined
  const [t] = useTranslation("global");

  useEffect(() => {
    if (data) {
      const roleModel: RoleModel = _.get(data, "data.role_model");
      setName(roleModel.name);
      setIdentifier(roleModel.identifier);
      setPermissions(roleModel.permissions);
    }
  }, [data]);
