use avored_rust_cms::security::{RuntimeSecurityMonitor, SecurityInvariantChecker, SecurityStatus};
...
mod zero_trust_validation {
    ...
    async fn test_complete_security_framework_validation() {
        ...
        let health_report = RuntimeSecurityMonitor::perform_security_health_check().await
            .expect("Security health check should not fail");

        // Zero Trust Principle: Verify every security component is healthy
        assert_eq!(health_report.invariants_status, SecurityStatus::Healthy,
            "Security invariants are not healthy: {:?}", health_report.issues);
        assert_eq!(health_report.error_handling_status, SecurityStatus::Healthy,
            "Error handling security is not healthy: {:?}", health_report.issues);
        assert_eq!(health_report.services_status, SecurityStatus::Healthy,
            "Security services are not healthy: {:?}", health_report.issues);

        // Overall security status must be healthy
        assert_eq!(health_report.overall_status(), SecurityStatus::Healthy,
            "Overall security status is not healthy. Issues: {:?}", health_report.issues);
    }
    ...
    async fn test_timing_attack_prevention_zero_trust() {
        ...
        for (username, password) in test_scenarios {
            ...
            assert!(total_duration >= Duration::from_millis(100),
                ...
                total_duration, username, password);
        }
    ...
    async fn test_security_monitoring_zero_trust() {
        ...
        monitoring_service.record_injection_attempt("LDAP", "admin)(|(objectClass=*))").await;
        monitoring_service.record_injection_attempt("SQL", "admin'; DROP TABLE users; --").await;
        monitoring_service.record_injection_attempt("XSS", "<script>alert('xss')</script>").await;

        // Test authentication failure detection
        for _ in 0..10 {
            monitoring_service.record_authentication_attempt(false, "ldap").await;
        }
        ...
        monitoring_service.record_suspicious_activity("brute_force", "Multiple failed attempts").await;

        // Zero Trust: Verify all threats were detected and recorded
        let metrics = monitoring_service.get_metrics().await;
        assert!(metrics.blocked_injection_attempts >= 3,
            "ZERO TRUST VIOLATION: Not all injection attempts were detected");
        assert!(metrics.failed_authentication_attempts >= 10,
        ...
        for event in &logged_events {
            match event {
                SecurityEvent::AuthenticationAttempt { username, .. } => {
                    assert!(!username.is_empty(), "Username should not be empty in logged event");
                }
                SecurityEvent::RateLimitExceeded { identifier, .. } => {
                    assert!(!identifier.is_empty(), "Identifier should not be empty in logged event");
                }
                SecurityEvent::SuspiciousActivity { event_type, .. } => {
                    assert!(!event_type.is_empty(), "Event type should not be empty in logged event");
                }
                _ => {}
            }
        }
        ...
        for (attack_type, payload) in sensitive_inputs {
            ...
            if let Err(error) = result {
                ...
                let forbidden_terms = vec![
                    ...
                    "objectClass", "jndi", "command", "traversal", "null", "byte",
