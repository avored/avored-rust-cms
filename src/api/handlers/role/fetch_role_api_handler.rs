...
#[cfg(test)]
mod tests {
    use crate::api::rest_api_routes::tests::{
        get_axum_app, get_login_response, send_get_request, setup_avored_db,
    };
    ...
    async fn test_fetch_role_api_handler() -> crate::error::Result<()> {
        ...
        let response = app
            .oneshot(send_get_request("/api/role", token))
            .await
            .unwrap();
        ...
        let body: RolePagination = serde_json::from_str(&body_str).expect("Failed to parse JSON");
