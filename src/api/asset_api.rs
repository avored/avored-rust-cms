...
impl Asset for AssetApi {
    async fn paginate(
        ...
        match self.state.asset_service.paginate(req, &self.state.db).await {
            Ok(reply) => {
    ...
    async fn create_folder(
        ...
        match self
            .state
            .asset_service
            .create_asset_folder(&self.state.db, req, &logged_in_user.email)
            .await
        {
            Ok(reply) => {
                let res = Response::new(reply);
    ...
    async fn delete_asset(
        ...
        match self
            .state
            .asset_service
            .delete_asset(&self.state.db, req)
            .await
        {
            Ok(reply) => {
                let res = Response::new(reply);
    ...
    async fn delete_folder(
        ...
        match self
            .state
            .asset_service
            .delete_folder(&self.state.db, req)
            .await
        {
            Ok(reply) => {
                let res = Response::new(reply);
    ...
    async fn rename_asset(
        ...
        match self
            .state
            .asset_service
            .rename_asset(&self.state.db, req, &claims.email)
            .await
        {
            Ok(reply) => {
                let res = Response::new(reply);
